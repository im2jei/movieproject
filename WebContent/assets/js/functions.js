...
$( function() {
  ...
  /*
	 * ! jQuery UI - v1.12.1 - 2016-09-14 http://jqueryui.com Includes:
	 * widget.js, position.js, data.js, disable-selection.js, effect.js,
	 * effects/effect-blind.js, effects/effect-bounce.js,
	 * effects/effect-clip.js, effects/effect-drop.js,
	 * effects/effect-explode.js, effects/effect-fade.js,
	 * effects/effect-fold.js, effects/effect-highlight.js,
	 * effects/effect-puff.js, effects/effect-pulsate.js,
	 * effects/effect-scale.js, effects/effect-shake.js, effects/effect-size.js,
	 * effects/effect-slide.js, effects/effect-transfer.js, focusable.js,
	 * form-reset-mixin.js, jquery-1-7.js, keycode.js, labels.js,
	 ...
	 * widgets/sortable.js, widgets/spinner.js, widgets/tabs.js,
	 * widgets/tooltip.js Copyright jQuery Foundation and other contributors;
	 * Licensed MIT
	 */
		...
		this._on( this.menu.element, {
			mousedown: function( event ) {
				...
				this._delay( function() {
					...
					// menu items will
					// result in focus moving out of the input. However, we've
					// already received
					// and ignored the blur event because of the cancelBlur flag
					// set above. So
					// we restore focus to ensure that the menu closes properly
					// based on the user's
					// next actions.
...
var widgetsControlgroup = $.widget( "ui.controlgroup", {
	...
	_initWidgets: function() {
		...
		$.each( this.options.items, function( widget, selector ) {
			...
			// We assume everything is in the middle to start because we can't
			// determine
			...
			that.element
				...
				.each( function() {
					...
					// We need to clone the default options for this type of
					// widget to avoid
					// polluting the variable options which has a wider scope
					// than a single widget.
	...
	refresh: function() {
		...
		// We filter here because we need to track all childWidgets not just the
		// visible ones
		...
		if ( children.length ) {
			...
			// We do this last because we need to make sure all enhancment is
			// done
...
$.widget( "ui.checkboxradio", [ $.ui.formResetMixin, {
	...
	_getCreateOptions: function() {
		...
		// We read the type here, because it makes more sense to throw a element
		// type error first,
		...
		// We need to get the label text but this may also need to make sure it
		// does not contain the
		// input itself.
		this.label.contents().not( this.element[ 0 ] ).each( function() {

			// The label contents could be text, html, or a mix. We concat each
			// element to get a
			// string representation of the label, without the input as part of
			// it.
	...
	_getRadioGroup: function() {
		...
		if ( this.form.length ) {
			...
			// Not inside a form, check all inputs that also are not inside a
			// form
	...
	_updateLabel: function() {
		...
		// Remove the contents of the label ( minus the icon, icon space, and
		// input )
...
$.widget( "ui.button", {
	...
	_create: function() {
		...
		// _getCreateOptions,
		// because null may have been passed on init which would override what
		// was set in
		// _getCreateOptions
		if ( this.options.disabled == null ) {
			...
			this._on( {
				"keyup": function( event ) {
					if ( event.keyCode === $.ui.keyCode.SPACE ) {
						...
						// If a native click is available use it so we actually
						// cause navigation
	...
	_updateIcon: function( option, value ) {
		...
		if ( displayBlock ) {
			...
			// Position is beginning or end so remove the ui-widget-icon-block
			// class and add the
	...
	_setOption: function( key, value ) {
		...
		if ( key === "label" ) {
			if ( this.isInput ) {
				...
				// If there is an icon, append it, else nothing then append the
				// value
	...
	refresh: function() {
		...
		// Make sure to only check disabled if its an element that supports this
		// otherwise
...
// select element, allowing it to be customizable in behavior and appearance far
// beyond the limitations of a native select.
// jscs:enable maximumLineLength
...
var widgetsSelectmenu = $.widget( "ui.selectmenu", [ $.ui.formResetMixin, {
	...
	_toggleAttr: function() {
		...
		// We can't use two _toggleClass() calls here, because we need to make
		// sure
		// we always remove classes first and add them second, otherwise if both
		// classes have the
...
// >>label: Tooltip
// >>group: Widgets
// >>description: Shows additional information for any element on hover or
// focus.
...
$.widget( "ui.tooltip", {
	...
	close: function( event ) {
		...
		if ( !tooltipData ) {
			...
			// We set ui-tooltip-open immediately upon open (in open()), but
			// only set the
			// additional data once there's actually content to show (in
			// _open()). So even if the
			// tooltip doesn't have full data, we always remove ui-tooltip-open
			// in case we're in
